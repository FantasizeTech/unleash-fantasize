...
beforeEach(async () => {
    ...
    app.use((req, _res, next) => {
        Object.assign(req, {
            user,
            audit: TEST_AUDIT_USER,
            checkRbac: async (requested: string[]) => {
...
test('creates multiple OSS projects, generated IDs and owner roles', async () => {
    ...
    expect(await fixture.projectStore.get('fantasizetech')).toMatchObject({
        name: 'FantasizeTech',
    });
...
test.each([
    ...
    { name: 'Bad', id: '../bad' },
    { name: 'Bad', mode: 'private' },
    { name: 'Bad', featureLimit: -1 },
    { name: 'Bad', environments: [] },
    { name: 'Bad', extra: true },
...
test('rejects missing environments and duplicate explicit ids', async () => {
    await request(app)
        .post('/projects')
        .send({ name: 'Missing', environments: ['nonexistent'] })
        .expect(400);
    await request(app)
        .post('/projects')
        .send({ name: 'One', id: 'same' })
        .expect(201);
    await request(app)
        .post('/projects')
        .send({ name: 'Two', id: 'same' })
        .expect(409);
});
...
test('updates project settings but cannot switch the authorized project id', async () => {
    await request(app)
        .post('/projects')
        .send({ name: 'One', id: 'one' })
        .expect(201);
    await request(app)
        .put('/projects/one')
        .send({ id: 'other', name: 'Attack' })
        .expect(400);
    await request(app)
        .put('/projects/one')
        .send({ name: 'Renamed', description: 'Updated' })
        .expect(200);
    expect(await fixture.projectStore.get('one')).toMatchObject({
        name: 'Renamed',
        description: 'Updated',
    });
    await request(app)
...
describe('project listing', () => {
    test.each([
        undefined,
        'true',
    ])('lists accessible projects, including non-default projects (archived=%s)', async (archived) => {
        ...
        expect(respondWithValidation.mock.calls[0][3].projects).toEqual(
            projects,
        );
    });
});
...
test('archives and revives a project but protects Default', async () => {
    await request(app)
        .post('/projects')
        .send({ name: 'Lifecycle', id: 'lifecycle' })
        .expect(201);
...
test('requires permissions for lifecycle and flag moves', async () => {
    ...
    await request(app)
        .post('/projects/one/features/flag/changeProject')
        .send({ newProjectId: 'two' })
        .expect(403);
});
